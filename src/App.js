  ...
  return (
    <div style={{ textAlign: "left", paddingBottom: "10px" }}>
      {message
        ...
        .split("")
        .map((letter) => {
          const cat_src = isDaddyMode(catmode)
            ? letter_to_daddy_cat[letter]
            : letter_to_og_cat[letter];
          return cat_src ? (
            <img
              height={FONT_SIZE}
              style={{ marginBottom: `-${FONT_MARGIN_BOTTOM}px` }}
              src={cat_src}
            />
...
function App() {
  ...
  async function onShare() {
    const shareableLink = `${SITE_PATH}/?${QUERY_PARAM_CATMODE}=${catmode}&${QUERY_PARAM_MSG}=${encodeURI(
      inputValue
    )}`;
